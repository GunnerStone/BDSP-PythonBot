...
def run(driver, controller):
    ...
    time.sleep(0.1)
    logging.debug("Navigating to old couple")
    ...
    def charge_vs_seeker():
        logging.debug("Charging VS Seeker")
    ...
    def locate_vs_seeker_alert_on_screen():
        ...
        if driver.match_image(img, "BDSP-Scripts/assets/vs_seeker_alert.png", threshold=0.01):
            logging.debug("VS Seeker Alert found on screen")
            return True
        else:
            logging.debug("VS Seeker Alert not found on screen")
            return False
    ...
    def check_for_fight():
        ...
        logging.debug("Scanning for VS Seeker Alert on screen")
    ...
    while num_PP > 0:
        ...
        if check_for_fight():
            ...
            time.sleep(0.1)
            logging.debug("Walking to old ldady")
            ...
            logging.debug("Pressing A until pokewatch disappears")
            ...
            logging.debug("Inside battle; Pressing A until the pokewatch reappears")
            # press A until the pokewatch reappears
            while not utils.has_pokewatch_btn_screen(driver):
                driver.hold_key(controller['Key_A'].lower(),0.2).wait(0.2)

            logging.debug("Pressing B to remove extra dialogue with old lady")            
